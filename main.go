...
func handleWriteBlock(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithJSON(w, r, http.StatusBadRequest, msg)
		return
	}

	if !isBlockValid(newBlock, prevBlock) {
		write(w, []byte("block validation failed"))
...
func respondWithJSON(w http.ResponseWriter, _ *http.Request, code int, payload interface{}) {
	...
	if err != nil {
		...
		write(w, []byte("HTTP 500: Internal Server Error"))
		return
	}
	w.WriteHeader(code)
	write(w, response)
...
func write(writer http.ResponseWriter, data []byte) {
	...
	if err != nil {
		log.Printf("failed to write bytes: %s\n", err)
	} else {
